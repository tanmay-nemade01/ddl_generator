...
ALTER TASK <env>_CDP_CMACGM.LARA.TS_LARA_<object name>_INGEST_MG  SUSPEND;

ALTER PIPE <env>_CDP_CMACGM.LARA_LANDING.SP_<object name> SET PIPE_EXECUTION_PAUSED = TRUE;

CREATE OR REPLACE TABLE <object name>_BACKUP CLONE <object name>;

CREATE OR REPLACE TRANSIENT TABLE LARA.CDC_<object name> AS 
SELECT <CDC_TARGET1>
...
SELECT * FROM <object name> WHERE FILE_DATE IS NULL;
...
select max(rowid) from CDC_<object name>;
...
if file is not None:
    ...
    pk_a_b = ''
    for i in range(int(data['No of Primary Key'][0])):
        pk_a_b = pk_a_b + f'a.{primary_key_list[i]} = b.{primary_key_list[i]} and\n'
    ...
    script_template = script_template.replace('<a-b>',pk_a_b[:-5])
