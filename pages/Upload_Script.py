...
comment = st.text_input('Enter Comment/Action')
...
-- Environment    : <env>
-- Project        : <SF_source> 
-- INGEST FLOW    : <object name>
-- INGESTION TYPE : FULL+CDC 
--=============Comments=======---------
-- Version	DATE		Name						ACTION
--  <version>		<date>	<user_name>		<comment>

USE ROLE <sio>_CDP_OFFICER;--FOR PROD PRD_CDP_OFFICER
USE WAREHOUSE <wh>;
...
if file is not None:
    ...
    script_template = script_template.replace('<version>',version)
    script_template = script_template.replace('<date>',date)
    script_template = script_template.replace('<user_name>',user_name)
    script_template = script_template.replace('<comment>',comment)
